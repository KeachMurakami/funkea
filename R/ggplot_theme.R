...
inward_tick <-
  function(length = 2, unit = "mm"){
    list(
      ggplot2::theme(axis.ticks.length = ggplot2::unit(-length, unit),
                     axis.ticks        = ggplot2::element_line(size = ggplot2::rel(.5)),
                     axis.text.x       = ggplot2::element_text(margin = ggplot2::unit(c(2*length, 0, 0, 0), units = unit)),
                     axis.text.y       = ggplot2::element_text(margin = ggplot2::unit(c(0, 2*length, 0, 0), units = unit)))
    )
  }
